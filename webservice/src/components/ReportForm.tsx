...
const ReportForm: React.FC<ReportFormProps> = ({ onReportSubmit, reportStatus }) => {
  ...
  return (
    <div className="mt-8 p-6 bg-light rounded-xl shadow-lg border border-gray-200 dark:bg-dark-800 dark:border-dark-700">
      <h3 className="text-xl font-bold text-dark mb-4 dark:text-light">Feedback & Reporting</h3>
      {!showForm ? (
        <button
          onClick={() => setShowForm(true)}
          className="w-full px-5 py-2.5 bg-primary text-white font-semibold rounded-md hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 transition-colors duration-200"
        >
